...
local settings_file_name = "settings.txt"
...
function on_player_joined_game(event)
  ...
  data = {
    prompt = player.mod_settings["prompt"].value,
    voice = player.mod_settings["voice"].value,
    mood = player.mod_settings["mood"].value,
    interval = player.mod_settings["interval"].value,
    max_words = player.mod_settings["word-limit"].value,
    openAI = player.mod_settings["openai"].value,
    azure = player.mod_settings["azureKey"].value,
    azureRegion = player.mod_settings["azureRegion"].value,
  }

  local settings_json = json.encode(data)
  local file = game.write_file(settings_file_name, settings_json, false, event.player_index)
...
function change_settings_init(event)
  for _, player in pairs(game.players) do
    ...
    local settings_json = json.encode(data)
    local file = game.write_file(settings_file_name, settings_json, false, player.index)
  end
end

function change_settings(event)
  local player = game.get_player(event.player_index)
...
script.on_init(change_settings_tick)
script.on_configuration_changed(change_settings_init)

script.on_event(defines.events.on_runtime_mod_setting_changed, change_settings)
...
-- script.on_event(defines.events.on_chunk_generated, on_chunk_generated) --Creates too much noise
